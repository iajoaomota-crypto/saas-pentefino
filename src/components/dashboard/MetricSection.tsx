...
import { Wallet, ArrowUpCircle, ArrowDownCircle, Scissors, TrendingUp, TrendingDown } from 'lucide-react';
import { Card } from '../ui';
...
export const MetricSection: React.FC<MetricSectionProps> = ({ stats, darkMode }) => {
    return (
        <div className="space-y-6">
            <Card className={cn(
                "p-8 relative overflow-hidden border-none shadow-2xl transition-all duration-300",
                darkMode ? "bg-gradient-to-br from-[#1E1E1E] to-[#0A0A0A]" : "bg-gradient-to-br from-[#00d26a] via-[#00b85c] to-blue-600"
            )}>
                {/* Visual Flair */}
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-center mb-10">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em] mb-3">Saldo Disponível</span>
                            <h2 className="text-4xl md:text-6xl font-black tracking-tight flex items-baseline gap-2 text-white">
                                ...
                                {stats.balance < 0 && <span className="text-red-300 text-xl ml-2 font-bold animate-pulse">(-)</span>}
                            </h2>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 md:gap-8 pt-8 border-t border-white/10">
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2 mb-1">
                                <div className="w-6 h-6 bg-[#00d26a]/20 rounded-md flex items-center justify-center text-[#00d26a]">
                                    <ArrowUpCircle size={14} />
                                </div>
                                <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">Entradas</span>
                            </div>
                        ...
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-2 mb-1">
                                <div className="w-6 h-6 bg-red-400/20 rounded-md flex items-center justify-center text-red-300">
                                    <ArrowDownCircle size={14} />
                                </div>
                                <span className="text-[10px] font-bold text-white/60 uppercase tracking-widest">Saídas</span>
                            </div>
